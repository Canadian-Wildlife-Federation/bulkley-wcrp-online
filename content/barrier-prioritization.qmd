...
A primary outcome of the WCRP will be the rehabilitation of barriers to connectivity in the Bulkley River Watershed. To achieve the goals in this plan, it is necessary to identify a set of structures that, if rehabilitated, will provide access to a minimum of 86 km of spawning habitat and 211 km of rearing habitat (@tbl-table16):
...
Thirty barriers on the intermediate list require short-term field assessments before selection as a final barrier to pursue for rehabilitation: 
...
Based on the results of the prioritization analysis, 30 barriers from the intermediate barrier list are required to be rehabilitated to achieve the connectivity goals in this plan:

```{r remcost, echo = FALSE, results = 'asis'}
#| label: tbl-remcost
#| tbl-cap: "Preliminary barrier rehabilitation cost estimate to reach connectivity goals in the Bulkley River watershed. Cost per barrier values are estimated based on the average cost of previously completed projects. Barrier counts and total costs are subject to change as more information is collected through the implementation of this plan."
#| warning: false
#| echo: false

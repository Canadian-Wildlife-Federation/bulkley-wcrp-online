...
total = api.watershed_connectivity("ALL")[1] #total km in BULK
access = api.watershed_connectivity("ALL")[2]
gain = round((total*0.95)-access,2)

df = pd.DataFrame({"Habitat Type":["Spawning","Rearing"],
                   "Currently accessible (km)":[str(access),"2"],
                   "Total": [str(total),"2"],
                   "Current Connectivity Status":[str(api.watershed_connectivity("ALL")[0])+"%","2"],
                   "Goal": ["95%","2"],

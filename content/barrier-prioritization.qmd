...
After all existing data and knowledge are collated for known and modelled crossings, an iterative ranking process is conducted to help identify barriers for rehabilitation to meet the WCRP goals. The ranking process is primarily used to guide field assessments and maximize efficiency in ground truthing data inputs and model outputs, while providing a secondary purpose to evaluate the relative key habitat gains of confirmed barriers in the watershed. This process, combined with input from local knowledge holders and experts, is used to develop field plans for assessing structures that have the potential to block the most key habitat in the watershed.
...
5.	Excluded structures list – the structure is excluded from further consideration in subsequent ranking and work planning because the structure is confirmed to bepassable, not present, or there is no key habitat upstream ([Appendix C](data-methods.qmd)).

...
def watershed_connectivity(habitat_type):
    ...
    request = 'https://cabd-pro.cwf-fcf.org/bcfishpass/functions/postgisftw.wcrp_habitat_connectivity_status/items.json?watershed_group_code=BULK&barrier_type=' + habitat_type
...
df = pd.DataFrame({"Barrier Types":["Road-Stream Crossings","Rail-stream Crossings", "Natural Barriers","Lateral Barriers","Large Dams(>3m height)","Small Dams(<3m height)","Trail-stream Crossings"],
                   "Extent":[rating("extent", "ROAD"), rating("extent", "RAIL"),"Medium", "Medium",rating("extent", "DAM"), rating("extent", "DAM"),rating("extent", "TRAIL")],
                   "Severity":[rating("severity", "ROAD"), rating("severity", "RAIL"),"Very High", "High",rating("severity", "DAM"),rating("severity", "RAIL"),rating("severity", "TRAIL")],
